...
#' }
theme_atropos <- function(base_family="Lato", base_size = 11.5,
  ...
  if (inherits(grid, "character") | grid == TRUE) {
    ...
    ret <- ret + ggplot2::theme(panel.grid=element_line(color=grid_col, linewidth=0.2))
    ret <- ret + ggplot2::theme(panel.grid.major=element_line(color=grid_col, linewidth=0.2))
    ret <- ret + ggplot2::theme(panel.grid.minor=element_line(color=grid_col, linewidth=0.15))
  ...
  if (inherits(axis, "character") | axis == TRUE) {
    ret <- ret + ggplot2::theme(axis.line=element_line(color="#2b2b2b", linewidth=0.15))
    if (inherits(axis, "character")) {
      axis <- tolower(axis)
      if (regexpr("x", axis)[1] < 0) {
        ret <- ret + ggplot2::theme(axis.line.x=element_blank())
      } else {
        ret <- ret + ggplot2::theme(axis.line.x=element_line(color=axis_col, linewidth=0.15))
      }
      if (regexpr("y", axis)[1] < 0) {
        ret <- ret + ggplot2::theme(axis.line.y=element_blank())
      } else {
        ret <- ret + ggplot2::theme(axis.line.y=element_line(color=axis_col, linewidth=0.15))
      }
    } else {
      ret <- ret + ggplot2::theme(axis.line.x=element_line(color=axis_col, linewidth=0.15))
      ret <- ret + ggplot2::theme(axis.line.y=element_line(color=axis_col, linewidth=0.15))
  ...
  if (!ticks) {
    ...
    ret <- ret + ggplot2::theme(axis.ticks = element_line(linewidth=0.15))
    ret <- ret + ggplot2::theme(axis.ticks.x = element_line(linewidth=0.15))
    ret <- ret + ggplot2::theme(axis.ticks.y = element_line(linewidth=0.15))
  ...
  ret <- ret + ggplot2::theme(axis.text.x=element_text(size=axis_text_size, margin=margin(t=0)))
  ret <- ret + ggplot2::theme(axis.text.y=element_text(size=axis_text_size, margin=margin(r=0)))
  ret <- ret + ggplot2::theme(axis.title=element_text(size=axis_title_size, family=axis_title_family))
  ret <- ret + ggplot2::theme(axis.title.x=element_text(hjust=xj, size=axis_title_size,
                                               family=axis_title_family, face=axis_title_face))
  ret <- ret + ggplot2::theme(axis.title.y=element_text(hjust=yj, size=axis_title_size,
                                               family=axis_title_family, face=axis_title_face))
  ret <- ret + ggplot2::theme(axis.title.y.right=element_text(hjust=yj, size=axis_title_size, angle=90,
                                                     family=axis_title_family, face=axis_title_face))
